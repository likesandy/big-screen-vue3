...
watch(
  () => props.panelItems,
  (newV) => {
    nextTick(() => setupCountUp(newV));
  }
);

onMounted(() => {
  setupCountUp(props.panelItems);
});

function setupCountUp(
  panelItems,
  options = [
    { decimalPlaces: 1, duration: 2, useGrouping: false },
    { decimalPlaces: 1, suffix: "%", duration: 2 },
  ]
) {
  panelItems.forEach((item) => {
    new CountUp(`total-num-${item.id}`, item.totalNum, options[0]).start();
    new CountUp(
